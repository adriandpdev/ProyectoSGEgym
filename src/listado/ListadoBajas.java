...
public class ListadoBajas extends JInternalFrame implements MouseListener,KeyListener, ActionListener{
	...
	public ListadoBajas() {
		...
		modelo_combo.addElement("Fecha de Baja");
	...
	public ArrayList<PersonaBaja> datarPersonasBusqueda(){
		...
		if(criterio == 0) {
			...
			sql_busqueda = "SELECT * FROM PersonaBaja WHERE fechabaja like '" + parametro + "%'";
		}
	...
	public void crearTabla(String[] titulos, Object[][] data) {
		...
		tablaPersonasBaja.setRowHeight(25);//tamaño de las celdas
		...
		tablaPersonasBaja.getColumnModel().getColumn(IndicadoresBaja.TELEFONO).setCellRenderer(new Celdas("campo"));//teléfono
		tablaPersonasBaja.getColumnModel().getColumn(IndicadoresBaja.CORREO).setCellRenderer(new Celdas("campo"));//correo electrónico
