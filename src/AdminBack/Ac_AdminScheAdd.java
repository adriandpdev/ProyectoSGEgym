...
public class Ac_AdminScheAdd implements ChangeListener, ActionListener {
			...
			try {
				if (!comprobarAula()) {
					...
					JOptionPane.showMessageDialog(null,
							"Hay una clase que ya está ocupando el mismo aula que la actividad elegida a esta hora");
				}
	...
	private void borrarHora() {
		if (v.getHoraDel().isEnabled() && v.getActividadDel().isEnabled() && v.getDiaDel().getSelectedIndex() != -1) {
			try {
				...
				ResultSet rs = con.consulta(Main.con, "Select idactividad from Actividad where nombre like '"
						+ v.getActividadDel().getSelectedItem() + "'");
				rs.next();
				con.alta(Main.con,
						"DELETE from Horario where idactividad=" + rs.getInt("idactividad") + " and hora like '"
								+ v.getHoraDel().getSelectedItem() + "' and diasemana like '"
								+ v.getDiaDel().getSelectedItem() + "'");
		...
		if (v.getHoraDel().isEnabled()) {
			...
			try {
				...
				ResultSet rs = con.consulta(Main.con,
						"select * from Horario h, Actividad a where h.idactividad=a.idactividad and h.diasemana like '"
								+ v.getDiaDel().getSelectedItem() + "' and h.hora like'"
								+ v.getHoraDel().getSelectedItem() + "'");
				while (rs.next()) {
	...
	private void rellenarHorasBorrar() throws SQLException {
		...
		try {
			...
			ResultSet rs = con.consulta(Main.con, "Select distinct(hora) from Horario where diasemana like '"
					+ v.getDiaDel().getSelectedItem() + "' order by hora asc");
			if (!rs.next()) {
				try {
	...
	private void insertarClase() throws SQLException {
		Conexion con = new Conexion();
		ResultSet rs = con.consulta(Main.con,
				"select idactividad from Actividad where nombre like '" + v.getActividad().getSelectedItem() + "'");
		rs.next();
		SimpleDateFormat formatter = new SimpleDateFormat("HH:mm:ss");
		Date hora = (Date) v.getHora().getValue();

		try {
			con.alta(Main.con, "INSERT INTO Horario VALUES (0, " + rs.getInt("idactividad") + ",'"
					+ v.getDia().getSelectedItem().toString() + "','" + formatter.format(hora) + "')");
	...
	private boolean comprobarAula() throws SQLException {
		...
		ResultSet rs = con.consulta(Main.con,
				"Select * from Actividad , Horario  where Actividad.idActividad=Horario.idActividad and Horario.Diasemana like '"
						+ v.getDia().getSelectedItem().toString() + "' and Horario.hora like '" + formatter.format(hora)
						+ "'");
